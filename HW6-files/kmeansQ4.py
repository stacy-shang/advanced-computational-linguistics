...
"""
I modified the program to work 3-dimensionally by incorporating the fundamental frequency values into how I calculated the means. 
I knew that F1 and F2 were the main features used to distinguish between vowels, as the former is inversely correlated with height and the latter 
is positively correlated with frontness. However, I was curious to see whether F0 would have any effect on the accuracy of the model, 
despite not being very important in disambiguiating vowels. Looking at the results, it seems the numbers only improved by ~0.1 for some cases, 
but often were worse off than the original 2-dimensional model. This indicates that F0 is likely not relevant to distinguishing vowels, 
and that the inclusion of F0 only added noise to the data.
"""

def mel(hertz):
    return(2595 * math.log10(1 + hertz / 700))

def dist(pt1: list, pt2: list):
    x1, y1, z1 = pt1
    x2, y2, z2 = pt2
...
while True:
    ...
    for line in open(voweldata): # loop thru each data pt in vowel data
        ...
        f0_curr = mel(float(row[3])) # grab f0 of token
        f1_curr = mel(float(row[4])) # grab f1 of token
        f2_curr = mel(float(row[5])) # grab f2 of token
        curr_token = [f0_curr, f1_curr, f2_curr] # put into list format [f1, f2] for dist calc
    ...
    for centroid in clusters:
        ...
        for token in clusters[centroid]:
            f0, f1, f2 = token[0], token[1], token[2]
            centroid_f0_values.append(f0)
            centroid_f1_values.append(f1)
            centroid_f2_values.append(f2)
        centroid_f0_mean = sum(centroid_f0_values) / len(centroid_f0_values)
        ...
        new_centroid = (centroid_f0_mean, centroid_f1_mean, centroid_f2_mean)
        new_means_list.append(new_centroid)

    # CONVERGENCE
    if means_list == new_means_list:
        # for mean in means_list:
        #     print(f'{mean[0]}       {mean[1]}       {mean[2]}')
        break
    else:
